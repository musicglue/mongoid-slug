    ...
    class UniqueSlug
      ...
      def find_unique(attempt = nil)
        MUTEX_FOR_SLUG.synchronize do
          ...
          scope_list = [
            uniqueness_scope.unscoped.where(where_hash),
            *@model.class.slug_other_scopes.map { |scope| scope.call(@model).where(where_hash) }
          ]
          ...
          if @state.slug_included?
            ...
            unless highest == 0 && model.class.slug_suffixless_first && @state.existing_slugs.count == 1
